...
void _updateCurrentTime() {
  ...
  Future<void> _initialize() async {
    ...
    print('Error initializing time tracking screen: $e');
    if (mounted) {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _refreshActiveTimeEntry() async {
    try {
      ...
      print('Error refreshing active time entry: $e');
    }
  }
  ...
  Future<void> _loadCurrentTimeEntryDetails() async {
    try {
      ...
      print('Error loading current time entry details: $e');
    }
  }
  ...
  //     final todayEntries = await ApiService.getTodayTimeEntries();
  //     final completedEntries = todayEntries.where((entry) => entry['isCompleted'] == true).toList();
      
  //     // Make sure all tasks are loaded for all categories to find tasks by ID
  //     for (var category in _taskCategories) {
  //       if (!_tasksMap.containsKey(category.id)) {
  //         await _loadTasksForCategory(category.id);
  //       }
  ...
  //       // Find the task in all loaded tasks
  //       Task? task;
  //       for (var categoryId in _tasksMap.keys) {
  //         final tasks = _tasksMap[categoryId] ?? [];
  //         for (var t in tasks) {
  //           if (t.id == taskId) {
  //             task = t;
  ...
  //       if (task != null) {
  //         if (taskDurations.containsKey(taskId)) {
  //           taskDurations[taskId] = taskDurations[taskId]! + duration;
  //         } else {
  ...
  //       _activeTasksDurations = taskDurations;
  //     });
  //     }
  //   } catch (e) {
  ...
  // Replace _loadActiveTasks method in time_tracking_screen.dart
...
Future<void> _loadActiveTasks() async {
  ...
  try {
    ...
    if (activeEntry != null && activeEntry['active'] == true) {
      ...
      if (todayData != null && todayData['timeEntries'] != null) {
...
Future<void> _clockIn({bool adjustedTime = false}) async {
  ...
  try {
    ...
    print('Verifying location for project: ${_selectedProject['id']}');
                  ...
                  onChanged: !_isClockedIn ? (value) {
                    if (value != null) {
                      ...
                      if (projectIndex >= 0) {
                        ...
                        print('Project with ID $value not found');
                      }
                    }
                  } : null,
                  hint: 'Select Project',
                    ...
                    onChanged: !_isClockedIn ? (value) {
                      if (value != null) {
                        ...
                        if (elevatorIndex >= 0) {
                          ...
                          print('Elevator with ID $value not found');
                        }
                      }
                    } : null,
...
Widget _buildDropdownWithCheck<T>({
  ...
  //                     onChanged: !_isClockedIn ? (value) {
  //                       setState(() {
  //                         _selectedElevator = _elevators.firstWhere((elevator) => elevator['id'] == value);
  //                       });
  //                     } : null,
  //                     hint: 'Select Elevator',
  //                   ),
                          ...
                          onChanged: (value) {
                            if (value != null) {
                              ...
                              if (taskIndex >= 0) {
                                ...
                                print('Task with ID $value not found');
                              }
                            }
                          },
                          hint: 'Select Task',
...
// Remove the original _buildDropdown method and solely use _buildDropdownWithCheck
  ...
  // Add this to the time_tracking_screen.dart file, replacing the _buildTaskSelectionSection() method
// Replace the _buildTaskSelectionSection method with this updated version

// Replace the _buildTaskSelectionSection method
...
//       // Select New Task Section (same as before)
//       Container(
//         padding: EdgeInsets.all(16),
//         margin: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
//         decoration: BoxDecoration(
...
//             // Task Category Selection (same as before)
//             _isTaskCategoriesLoading
//                 ? Center(child: CircularProgressIndicator(color: Color(0xFFF7D104)))
//                 : Container(
//                     height: 44,
...
//             // Task Selection and Switch Button (same as before)
//             Row(
//               children: [
//                 Expanded(
//                   child: _selectedCategory != null
...
//                                   }
//                                 },
//                                 hint: Text('Select Task', style: TextStyle(color: Colors.white70)),
//                                 dropdownColor: Color(0xFF2A2A2A),
//                                 isExpanded: true,
//                                 icon: Icon(Icons.arrow_drop_down, color: Color(0xFFF7D104)),
//                               ),
...
//             SizedBox(height: 8),
//             Text(
//               _selectedTask?.name ?? 'No Task Selected',
//               style: TextStyle(
//                 color: Color(0xFFF7D104),
//                 fontSize: 18,
//               ),
...
// Replace the _buildActiveTasksSection method

// Update the _buildActiveTasksSection method with correct type handling
...
Widget _buildActiveTasksSection() {
  ...
  //                     final duration = _activeTasksDurations[task.id] ?? Duration.zero;
  ...
  //                                 SizedBox(height: 4),
  //                                 Text(
  //                                   'Category: ${_getCategoryCodeForTask(task)}',
  //                                   style: TextStyle(
  //                                     color: Colors.white70,
  //                                     fontSize: 14,
  //                                   ),
...
//   Future<void> _updateActiveTask(Task newTask) async {
//     if (_isUpdatingTask) return;
...
//     try {
//       // Call API to update active time entry with the new task
//       final activeEntry = await ApiService.getActiveTimeEntry();
//       if (activeEntry != null) {
//         // Here you would call an API endpoint to update the active time entry
//         // This is a placeholder for the actual implementation
//         // await ApiService.updateActiveTimeEntry(activeEntry['id'], {'taskId': newTask.id});
...
//           _isUpdatingTask = false;
//         });
//         _showErrorSnackBar('No active time entry found');
//       }
//     } catch (e) {
//       setState(() {
//         _isUpdatingTask = false;
...
//             // Current Task Section (only when clocked in)
//             if (_isClockedIn)
//               _buildTaskSelectionSection(),
              
//             // Task Selection Section (always shown)
//             _buildTaskCategorySection(),
...
//         children: [
//           Text(
//             _isClockedIn ? 'Change Current Task' : 'Select Task',
//             style: TextStyle(
//               color: Colors.white,
//               fontSize: 16,
//               fontWeight: FontWeight.bold,
...
//                         )).toList(),
//                         onChanged: (value) {
//                           if (_isClockedIn && value != null && value.id != _selectedTask?.id) {
//                             // If clocked in and selected a different task, update active time entry
//                             _updateActiveTask(value);
//                           } else {
//                             setState(() {
//                               _selectedTask = value;
//                             });
...
//                           child: ElevatedButton(
//                             style: ElevatedButton.styleFrom(
//                               backgroundColor: _isUpdatingTask ? Colors.grey : Color(0xFFF7D104),
//                               foregroundColor: Colors.black,
//                               padding: EdgeInsets.symmetric(vertical: 12),
//                             ),
//                             onPressed: _isUpdatingTask ? null : () {
//                               if (_selectedTask != null) {
//                                 _updateActiveTask(_selectedTask!);
//                               } else {
//                                 _showErrorSnackBar('Please select a task first');
//                               }
//                             },
...
//           Text(
//             _selectedProject != null && _selectedElevator != null ? 
//               'Project: ${_selectedProject?['name'] ?? 'Unknown'} | Elevator: ${_selectedElevator?['number'] ?? 'Elevator ${_selectedElevator?['elevatorNumber'] ?? 'Unknown'}'}' :
//               'Project & Elevator Information Unavailable',
//             style: TextStyle(
//               color: Colors.white70,
//               fontSize: 14,
